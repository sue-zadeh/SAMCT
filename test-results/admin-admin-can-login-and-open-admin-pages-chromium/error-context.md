...
- Name: admin.spec.ts >> admin can login and open admin pages
- Location: client/tests/e2e/admin.spec.ts:3:1
...
```ts
  ...
  2  | 
  3  | test('admin can login and open admin pages', async ({ page }) => {
  4  |   await page.goto('/login')
  5  | 
> 6  |   await page.locator('#username').fill('secretary1')
     |                                   ^ Error: locator.fill: Test timeout of 30000ms exceeded.
  ...
  11 | 
  12 |   await page.getByRole('link', { name: /Village Data/i }).click()
  13 |   await expect(page).toHaveURL(/admin\/village-properties/)
  14 | 
  15 |   await page.getByRole('link', { name: /Maintenance/i }).click()
  16 |   await expect(page).toHaveURL(/admin\/maintenance/)
  17 | 
  18 |   await page.getByRole('link', { name: /Purchase Orders/i }).click()
  19 |   await expect(page).toHaveURL(/admin\/purchase-orders/)
  20 | })
```