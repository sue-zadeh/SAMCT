...
- Name: resident.spec.ts >> resident can login and open maintenance page
- Location: client/tests/e2e/resident.spec.ts:3:1
...
```ts
  ...
  2  | 
  3  | test('resident can login and open maintenance page', async ({ page }) => {
  4  |   await page.goto('/login')
  5  | 
> 6  |   await page.locator('#username').fill('resident1')
     |                                   ^ Error: locator.fill: Test timeout of 30000ms exceeded.
  7  |   await page.locator('#password').fill('123Suezx@')
  8  |   await page.getByRole('button', { name: /^login$/i }).click()
  9  | 
  10 |   await expect(page).toHaveURL(/resident/)
  11 | 
  12 |   await page.getByRole('link', { name: /Maintenance/i }).click()
  13 |   await expect(page).toHaveURL(/resident\/maintenance/)
  14 | })
```