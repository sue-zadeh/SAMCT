...
The original baseline is not ready for internet delivery with private resident data. This branch addresses the verified code issues below. Complete the deployment checks in the README before merging into a release and exposing the service.
...
| High | Upload validation was absent/inconsistent and had no per-file size limits. | Centralised generated filenames, image/document size limits, MIME/signature checks and Office archive checks. |
...
- React/TypeScript production build: passed locally.
- ASP.NET Core build: passed locally with zero warnings/errors.
- npm audit: zero known vulnerabilities at the time checked.
- NuGet vulnerable-package check, including transitives: no known vulnerabilities reported.
- Playwright suite: implementation added; the final execution result is recorded here after the CI run.
- Local environment limitations: standard browser download was unavailable; a local Chromium binary was obtained. The local PostgreSQL substitute was incompatible with Npgsql during migration. No passing E2E claim is based on that failed setup. The GitHub workflow uses PostgreSQL 17 and Playwright's managed Chromium.
